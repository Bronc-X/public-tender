...
type OtherLibraryHandler struct {
	db *sqlx.DB
}

func NewOtherLibraryHandler(db *sqlx.DB) *OtherLibraryHandler {
	return &OtherLibraryHandler{db: db}
}

// ListFolders returns all financial report folders for the current company
func (h *OtherLibraryHandler) ListFolders(c *gin.Context) {
	...
	err := h.db.Select(&folders, "SELECT * FROM other_folder WHERE company_id = ? ORDER BY created_at DESC", cid)
	if err != nil {
...
func (h *OtherLibraryHandler) CreateFolder(c *gin.Context) {
	companyID, exists := c.Get("companyID")
	if !exists {
		Error(c, http.StatusUnauthorized, "Missing company context")
		return
	...
	id := uuid.New().String()
	_, err := h.db.Exec("INSERT INTO other_folder (id, company_id, folder_name) VALUES (?, ?, ?)", id, cid, req.FolderName)
	if err != nil {
...
func (h *OtherLibraryHandler) DeleteFolder(c *gin.Context) {
	id := c.Param("id")
	companyID, _ := c.Get("companyID")
	cid := companyID.(string)

	_, err := h.db.Exec("DELETE FROM other_folder WHERE id = ? AND company_id = ?", id, cid)
	if err != nil {
...
func (h *OtherLibraryHandler) RenameFolder(c *gin.Context) {
	id := c.Param("id")
	companyID, _ := c.Get("companyID")
	cid := companyID.(string)
	...
	result, err := h.db.Exec("UPDATE other_folder SET folder_name = ? WHERE id = ? AND company_id = ?", req.FolderName, id, cid)
	if err != nil {
...
func (h *OtherLibraryHandler) ListFilesByFolder(c *gin.Context) {
	folderID := c.Param("id")
	companyID, _ := c.Get("companyID")
	cid := companyID.(string)
	...
	query := `
		...
		WHERE f.company_id = ? 
		  AND f.source_module = 'other_library'
