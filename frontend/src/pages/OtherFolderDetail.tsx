...
const OtherFolderDetail: React.FC = () => {
  ...
  const fetchFiles = async () => {
    setLoading(true);
    try {
      const res = await axios.get(`/api/others/folders/${folderId}/files`);
      setFiles(res.data || []);
    } catch (err) {
      console.error(err);
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24, padding: '0 0 24px 0' }} className="animate-in fade-in duration-500">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%' }}>
        <Space size="middle" style={{ flex: 1, minWidth: 0 }}>
          <Button icon={<LeftOutlined />} onClick={() => navigate('/library/others')} />
...
export default OtherFolderDetail;
