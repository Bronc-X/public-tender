...
import { FolderFilled, PlusOutlined, EllipsisOutlined, ExclamationCircleOutlined, AccountBookOutlined } from '@ant-design/icons';
...
const FinancialReportLibrary: React.FC = () => {
  ...
  const fetchFolders = async () => {
    setLoading(true);
    try {
      const res = await axios.get('/api/financial-reports/folders');
      setFolders(res.data || []);
    } catch (err) {
      console.error(err);
      message.error('加载财报夹失败');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCreateFolder = async (values: { folder_name: string }) => {
    setSubmitting(true);
    try {
      await axios.post('/api/financial-reports/folders', values);
  ...
  const handleDelete = (id: string, e: React.MouseEvent) => {
    ...
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          await axios.delete(`/api/financial-reports/folders/${id}`);
  ...
  const handleRename = async (id: string) => {
    ...
    try {
      await axios.patch(`/api/financial-reports/folders/${id}`, { folder_name: editingFolderName.trim() });
          ...
          {folders.length === 0 && !loading ? (
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '32px' }}>
              ...
              {folders.map(folder => (
                ...
                >
                  <div style={{ position: 'relative' }} onClick={() => navigate(`/library/financial-reports/${folder.id}?name=${encodeURIComponent(folder.folder_name)}`)}>
...
export default FinancialReportLibrary;
